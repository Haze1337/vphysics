...
static cell_t ApplyForceOffset(IPluginContext* pContext, const cell_t* params)
{
	IPhysicsEnvironment* pPhysicsEnvironment = iphysics->GetActiveEnvironmentByIndex(0);
	...
	pObject->ApplyForceOffset(forceVector, worldPosition);
...
static cell_t GetVelocity(IPluginContext* pContext, const cell_t* params)
{
	...
	if (params[4] > 0)
	{
		pObject->GetImplicitVelocity(&velocity, &angularVelocity);
	}
	else
	{
		pObject->GetVelocity(&velocity, &angularVelocity);
	}

	cell_t* addr;
	pContext->LocalToPhysAddr(params[2], &addr);
	addr[0] = sp_ftoc(velocity.x);
	addr[1] = sp_ftoc(velocity.y);
	addr[2] = sp_ftoc(velocity.z);

	cell_t* addr2;
	pContext->LocalToPhysAddr(params[3], &addr2);
	addr2[0] = sp_ftoc(angularVelocity.x);
	addr2[1] = sp_ftoc(angularVelocity.y);
	addr2[2] = sp_ftoc(angularVelocity.z);
...
BEGIN_NATIVES(Phys)
	...
	ADD_NATIVE(Phys, ApplyForceOffset)
